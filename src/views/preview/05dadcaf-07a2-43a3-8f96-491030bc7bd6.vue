...
class Sketch {
  ...
  setupCanvas() {
    ...
    containerRef.value.appendChild(this.canvas)
  }

  setupEvents() {
    window.addEventListener('resize', this.onResize.bind(this), false)
    window.addEventListener('wheel', this.onWheel.bind(this), false)
    containerRef.value.addEventListener('click', this.onClick.bind(this), false)
  ...
  setupSizes() {
    ...
    this.width = this.preWidth = this.canvas.width = width
  ...
  resetParams() {
    this.hover = false
    containerRef.value.style.cursor = 'initial'
  }
...
.loading {
  ...
  visibility: visible;
  transition:
    opacity 0.8s 0.8s cubic-bezier(0.755, 0.05, 0.855, 0.06),
