...
onMounted(() => {
  ...
  selectors.forEach((selector: HTMLElement) => {
    const knob = selector.querySelector('.knob') as HTMLElement
...
:where(main) {
  ...
  --shadow:
    rgba(0, 0, 0, 0.125) 0px 0px 2px -0.5px,
...
.selector {
  ...
  & ul {
    ...
    &:before {
      transition:
        --selector-width 0.3s ease,
        --color-accent-on-code 0.3s ease,
      ...
      box-shadow:
        var(--shadow),
        0 calc(var(--radius) / -10) 120px calc(var(--radius) / 5)
          rgb(0 0 0 / 0.05);
    ...
    &:after {
      ...
      width: calc(
        calc(100% + calc(var(--radius) / 9)) +
          calc(calc(var(--border-width) * 2) * 1px)
      );
      height: calc(
        calc(100% + calc(var(--radius) / 9)) +
          calc(calc(var(--border-width) * 2) * 1px)
      ...
      border-radius: 1e5px;
      box-shadow:
        0 0 calc(var(--is-active) * 2px) var(--color-shadow-darker),
        0 0 calc(var(--is-active) * 6px) var(--color-shadow),
    ...
    & li {
      ...
      &:before {
        --op: opacity(
          var(--is-selected)
        ); // need to use a var or the linter yells
        width: 100%;
        aspect-ratio: 1/1;
