<template>
  <button class="toggle" aria-pressed="true" @click="onToggle">
...
const onToggle = e => {
  const { target } = e
  const isDarkMode = target.matches('[aria-pressed=true]')
  const { classList } = target.parentNode.parentNode

  if (isDarkMode) {
    target.setAttribute('aria-pressed', false)
    classList.replace('bg-muted', 'bg-[hsl(219,30%,88%)]')
  } else {
    target.setAttribute('aria-pressed', true)
    classList.replace('bg-[hsl(219,30%,88%)]', 'bg-muted')
  }
}
</script>
